...
export async function installCommand(serverName: string, options: InstallOptions): Promise<void> {
  ...
  try {
    ...
    if (options.dryRun) {
      ...
      console.log(chalk.white(`  Command: ${server.installation.command} ${server.installation.args.join(' ')}`));
      return;
    }
    ...
    for (const clientType of targetClients) {
      ...
      try {
        ...
        await configEngine.installServer(
          clientInfo.configPath,
          server.id,
          mcpConfig,
          { backup: options.backup, force: options.force }
        );
        ...
        spinner.fail(chalk.red(`Failed to install to ${getClientDisplayName(clientType)}: ${errorMessage}`));
      }
    }
